...
namespace Notebook
{
    public partial class Form1 : Form
    {
        ...
        private int TabCount = 0;   //Tab Count
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void RemoveAllTabsButThis()     // Close all tabs except the open one
        {
            foreach(TabPage Page in tabControl1.TabPages)
            {
        ...
        private void Save()
        {
            // If the file has been saved before (FileName exists), it will be overwritten.
            if (string.IsNullOrEmpty(saveFileDialog1.FileName))
            {
                // If the file has not been saved before, the "Save As" window will open.
                SaveAs();
            }
            else
            {
                try
                {
                    // Saving is done according to the file extension
                    ...
                    {
                        GetCurrentDocument.SaveFile(saveFileDialog1.FileName, RichTextBoxStreamType.PlainText); // C# file is saved as plain text
                    }
                    else
                    {
                ...
                {
                    MessageBox.Show($"File could not be saved: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void SaveAs()
        {
            ...
            // We are showing the user which file types they can select
            saveFileDialog1.Filter = "RTF Files|*.rtf|Text Files|*.txt|C# Files|*.cs|All Files|*.*";
            saveFileDialog1.Title = "Save As";

            // If the user does not select a file or cancels, the FileName is reset.
            ...
            {
                // If the user presses "Cancel" the FileName will be reset
        ...
        private void Open()
        {
            ...
            openFileDialog1.Filter = "RTF|*.rtf|Text File|*.txt|C# File|*.cs|All Files|*.*";
            ...
            if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (openFileDialog1.FileName.Length > 0)
                {
                    try
                    {
                        ...
                        else if (fileExtension == ".txt" || fileExtension == ".cs")
                        {
                            // Upload .txt and .cs files as plain text
        ...
        #region
        private RichTextBox GetCurrentDocument  // Get Active Document
        {
        ...
        private void GetFontCollection()   // Loading fonts
        {
        ...
        private void PopulateFontSize()    // Create font scales
        {
            for(int i=1; i<=75; i++)
            {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                toolStripStatusLabel1.Text = "Total number of characters = " +
